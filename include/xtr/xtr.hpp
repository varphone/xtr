...
/// 一个代表数据包类型的枚举。
enum class XtrPacketType : uint8_t {
    ...
    XTR_PACKET_TYPE_VIDEO_AV1 = 13,
    XTR_PACKET_TYPE_VIDEO_AV2 = 14,
    XTR_PACKET_TYPE_VIDEO_AVS1 = 15,
    XTR_PACKET_TYPE_VIDEO_AVS2 = 16,
    XTR_PACKET_TYPE_VIDEO_AVS3 = 17,
    XTR_PACKET_TYPE_VIDEO_H264 = 18,
    XTR_PACKET_TYPE_VIDEO_H265 = 19,
...
/// 一个代表打包的值表的条目的迭代器的类型。
struct XtrPackedItemIter;

/// 一个代表打包的值表的类型。
struct XtrPackedValues;
...
using XtrClientRef = Arc<XtrClient>;

/// 一个代表 XTR 客户端实例可写指针的类型。
using XtrClientPtr = XtrClientRef*;
...
/// 一个代表数据包可写指针的类型。
using XtrPacketPtr = XtrPacketRef*;

/// 一个代表客户端数据包回调的类型。
using XtrClientPacketHandler = void(*)(XtrPacketPtr, void*);

/// 一个代表客户端状态的枚举。
using XtrClientState = XtrClientState;

/// 一个代表客户端状态回调的类型。
using XtrClientStateHandler = void(*)(XtrClientState, void*);

/// 一个代表打包的值表的只读指针的类型。
using XtrPackedValuesConstPtr = const XtrPackedValues*;

/// 一个代表数据包只读指针的类型。
using XtrPacketConstPtr = const XtrPacketRef*;

/// 一个代表打包的值表的可写指针的类型。
using XtrPackedValuesPtr = XtrPackedValues*;

/// 一个代表打包的值表的迭代器的可写指针的类型。
using XtrPackedItemIterPtr = XtrPackedItemIter*;
...
/// 一个代表数据包标志位的类型。
struct XtrPacketFlags {
    uint8_t bits;

    constexpr explicit operator bool() const {
        return !!bits;
    }
    constexpr XtrPacketFlags operator~() const {
        return XtrPacketFlags { static_cast<decltype(bits)>(~bits) };
    }
    constexpr XtrPacketFlags operator|(const XtrPacketFlags& other) const {
        return XtrPacketFlags { static_cast<decltype(bits)>(this->bits | other.bits) };
    }
    XtrPacketFlags& operator|=(const XtrPacketFlags& other) {
        *this = (*this | other);
        return *this;
    }
    constexpr XtrPacketFlags operator&(const XtrPacketFlags& other) const {
...
constexpr static const XtrPacketFlags XtrPacketFlags_END_STREAM = XtrPacketFlags{ /* .bits = */ (uint8_t)1 };
constexpr static const XtrPacketFlags XtrPacketFlags_END_HEADERS = XtrPacketFlags{ /* .bits = */ (uint8_t)4 };
...
/// # Safety
XtrPackedValuesPtr XtrPackedValuesNew();
